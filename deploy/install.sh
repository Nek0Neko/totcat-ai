#!/bin/bash
#
# TotCat Installation Script
# TotCat 安装脚本
# Usage: curl -sSL https://raw.githubusercontent.com/Nek0Neko/totcat-ai/main/deploy/install.sh | bash
#
...
# Configuration
GITHUB_REPO="Nek0Neko/totcat-ai"
INSTALL_DIR="/opt/totcat"
SERVICE_NAME="totcat"
SERVICE_USER="totcat"
CONFIG_DIR="/etc/totcat"
...
declare -A MSG_ZH=(
    ...
    # Completion
    ["install_complete"]="TotCat 安装完成！"
    ["install_dir"]="安装目录"
    ["next_steps"]="后续步骤"
    ["step1_check_services"]="确保 PostgreSQL 和 Redis 正在运行："
    ["step2_start_service"]="启动 TotCat 服务："
    ...
    ["not_installed"]="TotCat 尚未安装，请先执行全新安装"
    ["fresh_install_hint"]="用法"

    # Uninstall
    ["uninstall_confirm"]="这将从系统中移除 TotCat。"
    ...
    ["uninstall_complete"]="TotCat 已卸载"

    # Help
    ["usage"]="用法"
    ["cmd_none"]="(无参数)"
    ["cmd_install"]="安装 TotCat"
    ["cmd_upgrade"]="升级到最新版本"
    ["cmd_uninstall"]="卸载 TotCat"
    ...
    ["server_config_desc"]="配置 TotCat 服务监听地址"
...
declare -A MSG_EN=(
    ...
    # Installation
    ["install_title"]="TotCat Installation Script"
    ...
    # Completion
    ["install_complete"]="TotCat installation completed!"
    ["install_dir"]="Installation directory"
    ["next_steps"]="NEXT STEPS"
    ["step1_check_services"]="Make sure PostgreSQL and Redis are running:"
    ["step2_start_service"]="Start TotCat service:"
    ...
    ["not_installed"]="TotCat is not installed. Please run a fresh install first"
    ["fresh_install_hint"]="Usage"

    # Uninstall
    ["uninstall_confirm"]="This will remove TotCat from your system."
    ...
    ["uninstall_complete"]="TotCat has been uninstalled"

    # Help
    ["usage"]="Usage"
    ["cmd_none"]="(none)"
    ["cmd_install"]="Install TotCat"
    ["cmd_upgrade"]="Upgrade to the latest version"
    ["cmd_uninstall"]="Remove TotCat"
    ...
    ["server_config_desc"]="Configure TotCat server listen address"
...
get_current_version() {
    if [ -f "$INSTALL_DIR/totcat" ]; then
        # Use grep -E for better compatibility (works on macOS and Linux)
        "$INSTALL_DIR/totcat" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' | head -1 || echo "unknown"
    else
        echo "not_installed"
    fi
}
...
download_and_extract() {
    ...
    local archive_name="totcat_${version_num}_${OS}_${ARCH}.tar.gz"
    ...
    # Copy binary
    cp "$TEMP_DIR/totcat" "$INSTALL_DIR/totcat"
    chmod +x "$INSTALL_DIR/totcat"
    ...
    print_success "$(msg 'binary_installed') $INSTALL_DIR/totcat"
}
...
install_service() {
    ...
    cat > /etc/systemd/system/totcat.service << EOF
[Unit]
Description=TotCat - AI API Gateway Platform
Documentation=https://github.com/Nek0Neko/totcat-ai
...
User=totcat
Group=totcat
WorkingDirectory=/opt/totcat
ExecStart=/opt/totcat/totcat
Restart=always
RestartSec=5
StandardOutput=journal
StandardError=journal
SyslogIdentifier=totcat
...
start_service() {
    ...
    if systemctl start totcat; then
        print_success "$(msg 'service_started')"
        return 0
    else
        print_error "$(msg 'service_start_failed')"
        print_info "sudo journalctl -u totcat -n 50"
        return 1
    fi
}
...
enable_autostart() {
    ...
    if systemctl enable totcat 2>/dev/null; then
...
print_completion() {
    ...
    echo "=============================================="
    echo ""
    echo "  $(msg 'cmd_status'):   sudo systemctl status totcat"
    echo "  $(msg 'cmd_logs'):     sudo journalctl -u totcat -f"
    echo "  $(msg 'cmd_restart'):  sudo systemctl restart totcat"
    echo "  $(msg 'cmd_stop'):     sudo systemctl stop totcat"
    echo ""
    echo "=============================================="
}
...
upgrade() {
    ...
    # Get current version
    CURRENT_VERSION=$("$INSTALL_DIR/totcat" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' || echo "unknown")
    print_info "$(msg 'current_version'): $CURRENT_VERSION"

    # Stop service
    if systemctl is-active --quiet totcat; then
        print_info "$(msg 'stopping_service')"
        systemctl stop totcat
    fi

    # Backup current binary
    cp "$INSTALL_DIR/totcat" "$INSTALL_DIR/totcat.backup"
    print_info "$(msg 'backup_created'): $INSTALL_DIR/totcat.backup"
    ...
    # Set permissions
    chown "$SERVICE_USER:$SERVICE_USER" "$INSTALL_DIR/totcat"

    # Start service
    print_info "$(msg 'starting_service')"
    systemctl start totcat

    print_success "$(msg 'upgrade_complete')"
}

# Install specific version (for upgrade or rollback)
# Requires: TotCat must already be installed
install_version() {
    ...
    if [ -f "$INSTALL_DIR/totcat" ]; then
        ...
        if [ "$current_version" != "unknown" ] && [ "$current_version" != "not_installed" ]; then
            backup_name="totcat.backup.${current_version}"
        else
            backup_name="totcat.backup.$(date +%Y%m%d%H%M%S)"
        fi
        cp "$INSTALL_DIR/totcat" "$INSTALL_DIR/$backup_name"
...
uninstall() {
    ...
    print_info "$(msg 'stopping_service')"
    systemctl stop totcat 2>/dev/null || true
    systemctl disable totcat 2>/dev/null || true

    print_info "$(msg 'removing_files')"
    rm -f /etc/systemd/system/totcat.service
    systemctl daemon-reload
